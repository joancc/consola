...
Consola.Level1 = function() {};

//setting game configuration and loading the assets for the loading screen
Consola.Level1.prototype = {
    preload: function() {
        this.moving = false;
    ...
    create: function() {
        ...
        this.map = this.game.add.tilemap('level1');
            ...
            this.map.addTilesetImage('collectibles', 'collectibles');

		    //CREATE FROM TILED LAYERS
		    this.backgroundLayer = this.map.createLayer('BackgroundLayer');
		    
            //CREATE FROM TILED OBJECTS
		    //WALLS
		    ...
		    this.map.createFromObjects('ObjectBlockingLayer', 12, 'walls', 0, true, false, this.game.walls);
            ...
            this.map.createFromObjects('ObjectPressurePlateLayer', 509, 'desert', 33, true, false, this.game.pressurePlates, Consola.PressurePlate);
            ...
            this.game.collectibles = this.game.add.group();
                                        //(objectLayerName, gid, assetKey, spritesheetFrame, false, targetObject, 'instanceClass')
            this.map.createFromObjects('ObjectCollectibleLayer', 524, 'collectibles', 0, true, false, this.game.collectibles, Consola.Collectible);
            this.game.physics.arcade.enable(this.game.collectibles);    //Enable physics on group for overlap detection
            
            

            debug = this.game.collectibles;
            debug2 = this.pressurePlates;


		    //ENEMIES
		    this.game.enemies = this.game.add.group();
		    this.map.createFromObjects('ObjectEnemyLayer', 473, 'enemy', 0, true, false, this.game.enemies, Consola.Enemy);
        ...
        //ENEMIES
        Consola.Level1.enemies = [];
        // Consola.Level1.enemies = game.add.group();
        // Consola.Level1.enemies.add.existing(enemy);

        // enemy = new Consola.Enemy(this.game, 64, 64, 'enemy');
        // Consola.Level1.enemies.push(enemy);
    ...
    update: function() {
    	...
    	this.game.physics.arcade.overlap(player, this.game.enemies, this.handleEnemyCollision, null, this);
